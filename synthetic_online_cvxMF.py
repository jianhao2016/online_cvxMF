...
from visualization_NMF import plot_diff_method_new
import sys
...
if __name__ == '__main__':
    ...
    # ----------------------------------------------------
    # 2) online cvxMF, our algorithm. Rr
    ...
    print(' ------ ocmf final accuracy = {:.4f}, AMI = {:.4f}'.format(acc_Rr, AMI_Rr))

    df_centroids_Rr = pd.DataFrame(D_final_Rr.T, columns = pca_cols)
    df_centroids_Rr['label'] = ['ocmf: type {}'.format(x) for x in range(1, k_cluster + 1)]

    df_centroids_Rr.to_pickle('results_logging/ocmf_centroid_df')

    df_x_hat_Rr = pd.DataFrame(X_hat_Rr.T, columns = pca_cols)
    X_hat_set = ['group {}'.format(i) for i in range(k_cluster)]
    X_hat_label = []
    for idx in range(k_cluster):
        X_hat_label += [X_hat_set[idx]] * repre_size_count_Rr[idx]
    # ipdb.set_trace()
    df_x_hat_Rr['label'] = X_hat_label
    df_x_hat_Rr.to_pickle('results_logging/x_hat_df')
    
    # ----------------------------------------------------
    # 2.5) online cvxMF, our algorithm. Ru
    n_hat = k_cluster * candidate_set_size
    t_ocmf_Ru = 0
    acc_Ru = 0
    acc_array = []
    ...
    X_hat_label = []
    for idx in range(k_cluster):
        X_hat_label += [X_hat_set[idx]] * repre_size_count_Ru[idx]
    # ipdb.set_trace()
    df_x_hat_Ru['label'] = X_hat_label
    df_x_hat_Ru.to_pickle('results_logging/x_hat_df')
    ...
    # D_0 = np.absolute(D_0)
    # ipdb.set_trace()
    D_0 = (X_0_Rr @ W_0_Rr).reshape(m_dim, k_cluster)
    ...
    df_final['label'] = Y
    df_final = df_final.append(df_x_hat_Rr)
    ...
    df_final = df_final.append(df_centroids_Rr)
    ...
    accuracy_dict = {
            'nmf':[acc_nmf, AMI_nmf],
            'omf': [acc_omf, AMI_omf],
            'cmf': [acc_cnmf, AMI_cnmf],
            'ocmf_rr': [acc_Rr, AMI_Rr],
            'ocmf_ru': [acc_Ru, None]
            }
    ...
    fig = plot_diff_method_new(df_final, pca_cols, n_dim, k_cluster, 
            accuracy_dict, repre_size_count_Rr, repre_size_count_Ru,
